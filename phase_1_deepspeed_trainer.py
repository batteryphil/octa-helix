...
_smoke_test_done    = False   # set True after first successful save; prevents repeat
...
# ─────────────────────────────────────────────────────────────────────────────
def save_checkpoint(save_dir, phase, step, model_engine, optimizer, reason="periodic"):
    ckpt_path = os.path.join(save_dir, f"phase_{phase}.pt")
    tmp_path  = ckpt_path + ".tmp"
    ds_path   = ckpt_path + ".ds"
    print(f"\n[CKPT] Saving DeepSpeed checkpoint at step {step} ({reason})...")
    
    import deepspeed
    
    # In ZeRO-3, parameters are partitioned across GPUs (or offloaded).
    # We must gather them before calling state_dict().
    # GatheredParameters context automatically pulls everything to RAM/VRAM,
    # and then safely clears it when exiting the block.
    with deepspeed.zero.GatheredParameters(model_engine.module.parameters(), modifier_rank=0):
        # We only save on rank 0
        if deepspeed.comm.get_rank() == 0:
            raw_sd = model_engine.module.state_dict()
            state = {
                'model': raw_sd,
                'step':  step,
                'phase': phase,
    ...
    # ── Archival Copy (keep last 3 only) ─────────────────────────────────────
    # Saves a step-stamped copy for recovery, but prunes old ones.
    # At 5.8GB/checkpoint every 200 steps, keeping all = 1TB+. Keep 3 = ~17GB max.
    if reason == "periodic":
        import shutil, glob
        if os.path.exists(ckpt_path):
            ...
            # Prune: keep only the 3 most recent archival copies
            pattern = ckpt_path.replace('.pt', '_step_*.pt')
            old_ckpts = sorted(glob.glob(pattern), key=os.path.getmtime)
            for old in old_ckpts[:-3]:   # delete all but the 3 newest
                try:
                    os.remove(old)
                    print(f"[CKPT] Pruned old archival: {os.path.basename(old)}")
                except OSError:
                    pass
...
def train():
    ...
    try:
        ...
        if phase == '3r':
            ...
            print(f"Using AdamW for DeepSpeed — moments managed by ZeRO-3 CPU offload ({trainable_M:.0f}M trainable params).")
    except ImportError:
    ...
    # Ensure model is fully BFloat16 before handing to DeepSpeed, 
    # as the checkpoint loaded might contain FP32 weights.
    ...
    for step, batch in enumerate(dataloader_generator, start=resume_step):
        ...
        if step >= target_steps:
            ...
            save_checkpoint(save_dir, phase, step, model_engine, optimizer, reason="phase_complete")
            break
        ...
        # DeepSpeed handles bfloat16 optimization, but we still need autocast because ZeRO-3 often 
        # keeps LayerNorm weights in fp32 for stability, and PyTorch LayerNorm requires matching dtypes 
        # without an explicit autocast context.
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
            logits, domain_loss = model_engine(input_ids, loop_idx=0, domain_ids=domain_ids)
        ...
        lm_loss = loss # For telemetry mapping
        dom_l = domain_loss
        grad_norm = 0.0 # Handled internally by DeepSpeed
        ...
        batch_sz        = input_ids.size(0) if 'input_ids' in locals() else 1
        tps             = (seq_len * batch_sz * GRAD_ACCUM) / step_elapsed if step_elapsed > 0 else 0.0
        step_start_time = time.time()
        ...
        if step_elapsed > DATA_TIMEOUT:
            ...
            save_checkpoint(save_dir, phase, step + 1, model_engine, optimizer, reason="watchdog_stall")
            sys.exit(1)
        ...
        grad_norm_val = 0.0 # DeepSpeed handles logging if enabled
        log_line = (
        ...
        if should_stop:
            print(f"\n[AUTO-STOP] {reason}")
            save_checkpoint(save_dir, phase, step + 1, model_engine, optimizer, reason="auto_stop_divergence")
            with open(log_path, "a") as lf:
        ...
        # ── Early-save smoke test (first step after resume) ───────────────────
        # Proves the checkpoint save works within ~30 seconds of startup.
        # If this crashes, we know immediately — not 12 hours later.
        if not _smoke_test_done:
            _smoke_test_done = True
            print(f"\n[CKPT] Early-save smoke test at step {step+1}...")
            try:
                save_checkpoint(save_dir, phase, step + 1, model_engine, optimizer, reason="smoke_test")
                print(f"[CKPT] Smoke test PASSED - checkpoint save is working correctly")
            except Exception as e:
                print(f"[CKPT] Smoke test FAILED: {e}")
                print(f"[CKPT] STOPPING NOW - fix save_checkpoint before running again")
                raise

        # ── Periodic checkpoint every 200 steps ───────────────────────────────
        if (step + 1) % SAVE_EVERY == 0:
            save_checkpoint(save_dir, phase, step + 1, model_engine, optimizer, reason="periodic")

        # ── Graceful shutdown (SIGTERM / SIGINT) ─────────────────────────────
        if _shutdown_requested:
            print(f"\n[SHUTDOWN] Saving checkpoint at step {step+1} and exiting cleanly.")
            save_checkpoint(save_dir, phase, step + 1, model_engine, optimizer, reason="graceful_shutdown")
            with open(log_path, "a") as lf:
