...
def get_dataloader_for_phase(phase, tokenizer, resume_step=0, seq_len=512):
    ...
    if phase == '1':
        ...
        ds_c4       = load_dataset("allenai/c4", "en", split="train", streaming=True)
    ...
    # ── Phase 3 & 3j: Cognitive Bloom (chat + domain mix) ──────────────────────
        ...
        ds_arc      = load_dataset("allenai/ai2_arc", "ARC-Challenge", split="train", streaming=True)
        ds_wiki     = load_dataset("wikimedia/wikipedia", "20231101.en", split="train", streaming=True)
        ds_cnn      = load_dataset("abisee/cnn_dailymail", "3.0.0", split="train", streaming=True)
        ds_stories  = load_dataset("roneneldan/TinyStories",      split="train", streaming=True)
        ds_webtext  = load_dataset("Skylion007/openwebtext",       split="train", streaming=True)
...
def get_previous_phase(phase):
    return {'1': None, '2': '1', '3': '2', '3j': '3', '3r': '3j'}.get(phase)
        ...
        for prompt in WORD_SALAD_PROMPTS:
            try:
                ...
                for _ in range(MAX_TOKENS):
                    ctx       = generated[-CONTEXT_WINDOW:]
                    id_tensor = torch.tensor([ctx], dtype=torch.long, device=device)
...
def train():
    ...
    parser.add_argument('--phase', type=str, required=True, choices=['1', '2', '3', '3j', '3r'])
    ...
    save_dir     = cmd_args.save_dir if cmd_args.save_dir else os.path.join(project_dir, "titan_checkpoints")
    ...
    model = Mamba3Titan(vocab_size=50304, d_model=2048, n_layers=48,
    ...
    try:
        ...
        if phase == '3r':
            ...
            optimizer = bnb.optim.Adam8bit([
                {'params': router_params_list, 'lr': BASE_LR_ROUTER},  # learns fast
                {'params': head_params_list,   'lr': BASE_LR_HEAD},    # moderate
                {'params': core_params_list,   'lr': BASE_LR_CORE},    # arms: slow + careful
            ], weight_decay=0.01)
            print(f"3r co-training: router LR={BASE_LR_ROUTER:.0e} | "
                  f"arms LR={BASE_LR_CORE:.0e} | head LR={BASE_LR_HEAD:.0e}")
        elif phase in ('3j', 'sft') and trainable_M < 800:
        ...
        if phase == '3r':
            optimizer = torch.optim.AdamW([
                {'params': router_params_list, 'lr': BASE_LR_ROUTER},
                {'params': head_params_list,   'lr': BASE_LR_HEAD},
                {'params': core_params_list,   'lr': BASE_LR_CORE},
            ], weight_decay=0.01)
        else:
    ...
    if phase == '2':
        print("[CLONE] Phase 2 active: Enforcing 16 identical clones (copying Arm 0 to 1-15)...")
        with torch.no_grad():
    ...
    if phase == '1':
        ...
        target_steps = 60_000   # 60K × 512 = 30M domain tokens (was 30K = 15M — too short)
        seq_len      = 512
