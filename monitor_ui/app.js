...
const ARM_NAMES = [
    'General','Math','Logic','Code',
    'Factual','Summ','Creative','Instruct'
];
...
function updateArmBars(weights) {
    if (!weights || weights.length !== 8) return;
...
async function fetchTelemetry() {
    try {
        ...
        if (t.arm_sims && t.arm_sims.length === 8) {
