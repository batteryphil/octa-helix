...
# ─────────────────────────────────────────────────────────────────────────────
# Singleton — one engine per process, loaded once and kept warm in VRAM.
# Use get_engine() everywhere instead of constructing TitanInference() directly.
# ─────────────────────────────────────────────────────────────────────────────
_singleton_engine: "TitanInference | None" = None
...
def preload(checkpoint: str = "auto") -> None:
    """Eagerly load Titan into VRAM at process start — call this once at boot."""
    get_engine(checkpoint)
    print("[Titan] Preloaded and warm — zero-latency inference ready.")
...
class TitanInference:
    ...
    def stream(
        ...
        end_think_id = self.tokenizer.convert_tokens_to_ids("</think>")
        
        # Snapshot original model parameters to prevent permanent mutation across session
        orig_router_bias = self.model.domain_router.bias.data.clone()
        orig_router_temp = self.model.router_temp.data.clone()

        # Apply Helix arm router bias (additive to gate logits)
        if arm_bias is not None:
            import torch as _torch
            bias_tensor = _torch.tensor(
                arm_bias[:self.model.domain_router.bias.shape[0]],
                dtype=self.model.domain_router.bias.dtype,
                device=self.model.domain_router.bias.device,
            )
            self.model.domain_router.bias.data += bias_tensor
        ...
        try:
            ...
            self.model.router_temp.data.copy_(orig_router_temp)
